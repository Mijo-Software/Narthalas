...
namespace Narthalas
{
	...
	public partial class EssinForm : Form
	{
		...
		private static string GenerateLettersName(string pattern = DefaultPattern)
		{
		...
		private void BackgroundWorkerLetterNames_DoWork(object sender, System.ComponentModel.DoWorkEventArgs e)
		{
			...
			for (int i = 0; i < kryptonNumericUpDownSetLettersNumberNames.Value; i++)
			{
				if (kryptonRadioButtonWordLengthVariable.Checked)
				{
					...
					randomPattern = CreateRandomLetterPatternWithRandomMinAndMaxLength(minLength: minLength, maxLength: maxLength);
				...
				else if (kryptonRadioButtonWordLengthFixed.Checked)
				{
					...
					randomPattern = CreateRandomLetterPatternWithLength(length: fixedLength);

					kryptonTextBoxLettersResults.Text = string.IsNullOrWhiteSpace(value: kryptonTextBoxLettersResults.Text)
						? $"{quote}{GenerateLettersName(pattern: randomPattern, fixedLength: fixedLength)}{quote}"
						: $"{kryptonTextBoxLettersResults.Text}{separator}{newLine}{quote}{GenerateLettersName(pattern: randomPattern, fixedLength: fixedLength)}{quote}";
					ScrollToTextBoxLettersResultEnd();
				}
